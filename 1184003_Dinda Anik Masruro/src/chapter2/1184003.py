# -*- coding: utf-8 -*-
"""
Created on Mon Mar 15 13:09:05 2021
...
anggur = pd.read_csv('student-mat.csv', sep=';') 
# membaca file csv dengan method read_csv dari lib pandas dimana argumen pertama 
#nama file csv dan argumen kedua pemisah tiap datanya, lalu ditampung pada variable anggur
len(anggur) 
# mengetahui jumlah data pada list
#%% Soal 2
# generate binary label (pass/fail) based on G1+G2+G3
# (test grades, each 0-20 pts); threshold for passing is sum>=30
anggur['pass'] = anggur.apply(lambda row: 1 if (row['G1']+row['G2']+row['G3']) >= 35 else 0, axis=1) 
# memanggil method apply dari lib pandas untuk menerapkan function yang dibuat dengan lambda dimana parameternya adalah row, kemudian di cek apakah penjumlahan dari row['G1'], row['G2'], dan row['G3'] lebih dari sama dengan 35 maka akan mengembalikan 1 jika tidak maka mengembalikan 0 pada kolom pass, serta diterapkan di semua baris data
anggur = anggur.drop(['G1', 'G2', 'G3'], axis=1) 
# menghapus kolom G1, G2, dan G3
anggur.head() 
# menampilkan lima baris pertama datanya
#%% Soal 3
# use one-hot encoding on categorical columns
anggur = pd.get_dummies(anggur, columns=['sex','school','address','famsize','Pstatus','Mjob','Fjob','reason','guardian','schoolsup','famsup','paid','activities','nursery','higher','internet','romantic']) 
# memanggil method get_dummies untuk membuat variabel dummy dari data anggur dimana variable dummy yang dibuat adalah kolom 'sex','school','address','famsize','Pstatus','Mjob','Fjob','reason','guardian','schoolsup','famsup','paid','activities','nursery','higher','internet', dan 'romantic'
anggur.head() 
# menampilkan lima baris pertama datanya
#%% Soal 4
# shuffle rows
anggur = anggur.sample(frac=1) 
...
anggur_train = anggur[:300] 
# menampung 300 data pertama yang ditampung di variabel anggur_train
anggur_test = anggur[300:] 
# menampung sisa dari 300 data pertama yang ditampung di variabel anggur_test
anggur_train_att = anggur_train.drop(['pass'], axis=1) 
# menghapus kolom pass pada variabel anggur_train
anggur_train_pass = anggur_train['pass'] 
# menampung isi kolom pass pada variabel anggur_test di variabel anggur_train _pass
anggur_test_att = anggur_test.drop(['pass'], axis=1) 
# menghapus kolom pass pada variabel anggur_test
anggur_test_pass = anggur_test['pass'] 
# menampung isi kolom pass pada variabel anggur_test di variabel anggur_test _pass
anggur_att = anggur.drop(['pass'], axis=1) 
# menghapus kolom pass pada variabel anggur
anggur_pass = anggur['pass'] 
# menampung isi kolom pass di variabel anggur
# number of passing students in whole dataset:
import numpy as np # mengimport lib numpy lalu dialiaskan sebagai np
print("Passing: %d out of %d (%.2f%%)" % (np.sum(anggur_pass), len(anggur_pass),100*float(np.sum(anggur_pass))/len(anggur_pass))) 
...
fuji = fuji.fit(anggur_train_att, anggur_train_pass) 
...
dot_data = tree.export_graphviz(fuji, out_file=None, label="all",impurity=False, proportion=True,feature_names=list(anggur_train_att),class_names=["fail","pass"],filled=True, rounded=True) 
# memanggil method export_graphiz untuk mengekspor decision tree ke format DOT dan ditampung di variabel dot_data dengan argumen pertama decision tree yang di ekspor, argumen kedua nama file DOT dibiarkan default, argumen ketiga untuk menampilkan semua labelnya, argumen keempat impurity tidak ditampilkan, argumen kelima proportion ditampilkan presentase samplenya, argumen keenam nama fiturnya sesuai dengan variabel anggur_train_att, argumen ketujuh nama class/target adalah fail dan pass, argumen kedelapan filled adalah True untuk menujukkan tipenya, dan argumen kesembilan tampilan sudut kotaknya lingkaran
ungu = graphviz.Source(dot_data) 
# memanggil Source dengan argumen contructor sumber data DOT dan ditampung di variabel merangin
ungu
print (ungu)
# mencetak variabel
#%% Soal 7
# save tree
tree.export_graphviz(fuji, out_file="student-performance.dot",label="all", impurity=False,proportion=True,feature_names=list(anggur_train_att),class_names=["fail", "pass"],filled=True, rounded=True)
...
#%% Soal 8
fuji.score(anggur_test_att, anggur_test_pass) 
# memanggil method score untuk menghitung rata-rata akurasi
#%% Soal 9
from sklearn.model_selection import cross_val_score
hijau = cross_val_score(fuji, anggur_att, anggur_pass, cv=5) 
